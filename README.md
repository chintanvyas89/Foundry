...
| 🛠️ **`/implement` execution engine** | Every `@codebase`/`/plan` answer ends with an **Implement here (Foundry)** button. `/implement` compiles the plan into a deterministic Workflow IR and runs it in the **Foundry Execution** view: review each step in VS Code's native diff, then **Approve / Skip / Apply all**, with a one-click **Undo all** — no re-discovery, all on the local index. A **Continue in agent mode** button remains as a manual escape. |
...
## Project standards (PHP/PSR-4, frameworks) — pluggable

Foundry auto-detects a project's **standards** and uses them for local intelligence,
by **reading the ecosystem's own artifacts rather than reinventing the rules**:

- **PSR-4 namespace → directory map + framework** from Composer's generated
  `vendor/composer/installed.json` and `autoload_classmap.php` (authoritative, after
  `composer install`), the root `composer.json`, and Drupal `*.info.yml` markers
  (module namespaces Drupal registers at runtime, which aren't in Composer's PSR-4).
- **Coding standard** from the project's own linter config (`phpcs.xml`,
  `.php-cs-fixer`, `.editorconfig`).
...
— add a standard/ecosystem by dropping in a reader file and one line in `index.ts`. And
**any project can declare its own** standards with no code via a
...
Non-PHP ecosystems (e.g. TypeScript `tsconfig` `paths`) aren't built in yet, but the
same reader interface and `.foundry/standards.json` already cover them.
