...
| 🛠️ **`/implement` execution engine** | Every `@codebase`/`/plan` answer ends with an **Implement here (Foundry)** button. `/implement` compiles the plan into a deterministic Workflow IR and applies it in-extension with per-step approval, an auto-apply toggle, and a one-click **Undo all** — no re-discovery, all on the local index. An **Implement in agent mode** button remains as a manual escape. |
