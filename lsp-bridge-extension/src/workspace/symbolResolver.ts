...
//
// This is the anchor that lets edit operations survive earlier edits: the
// WorkspaceExecutor calls this per operation, right before building the edit, so
// ranges are always computed against the live document rather than precomputed
// (which would rot the moment a prior op shifts the file). Uses the same
// `vscode.executeDocumentSymbolProvider` path as symbolProvider.ts, so it works
// for any language with a symbol provider and needs no re-index.
...
export interface ResolvedSymbol {
  ...
  name: string;
  // Whole declaration incl. body — the target for replace_function/method/block.
  fullRange: vscode.Range;
  // Just the name identifier — the anchor for insert_before/after and rename.
  ...
  if (candidates.length > 1) {
    if (target.index === undefined) {
      const where = candidates
        .map((c) => `line ${c.selectionRange.start.line + 1}`)
        .join(', ');
      throw new SymbolResolutionError(
        'ambiguous_symbol',
